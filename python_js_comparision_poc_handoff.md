...
模型會輸出 50 維的 sigmoid 分數(對應 50 個音樂標籤)。我們比對兩端的平均分數、Top-K 標籤、相關係數,判定是否一致(PASS / FAIL)。

目的:確認「同一個模型在 Python 跟瀏覽器 / Node 環境跑出來的結果可以對得起來」,作為之後把推論搬到前端的依據。
...
`run_test.sh` 的流程:`preprocess`(ffmpeg 轉 16kHz 單聲道 wav)→ Python 推論 → JS 推論 → 比對 → 印報告。

產出檔案在 `results/`:

| 檔案 | 內容 |
|---|---|
| `python_output.json` | Python 端 50 維平均分數 + Top-10 標籤 |
| `js_output.json` | JS 端對應輸出(相同格式) |
| `comparison_report.txt` | 逐標籤差異統計、Jaccard、Spearman、PASS/FAIL 判定 |
...
MusiCNN 把 mel 頻譜切成一段段 187 frames 的 patch 餵給模型。重點在「每個 patch 往前跳幾個 frame」:
...
| JS n_patches | 80 | **79** |
| max \|diff\|(逐標籤最大差) | 0.0057 | **0.0015** |
| Jaccard@10(Top-10 標籤重疊度) | 0.82 | **1.0000** |
| Spearman ρ(50 維相關係數) | 0.9985 | **0.9998** |
| n_patches 不一致 warning | 有 | **無** |
| 判定 | PASS | **PASS(strict)** |

兩端現在做的事情完全一樣:16kHz 單聲道 → mel 頻譜 → 79 個不重疊 patch → 對 patch 取平均。
...
- **Mel 頻譜計算**:Python 用 essentia 的 C++ 實作,JS 用 essentia.js 編出來的 WASM。同樣演算法、不同 toolchain,浮點數會有微小差。
- **推論 backend**:Python 走 libtensorflow(C++),JS 刻意走純 JS 的 tfjs CPU backend(啟動時硬性 assert backend 是 `cpu`)。我們**故意不裝** `@tensorflow/tfjs-node`,避免 native backend 偷偷接管 —— 那會讓數值「太接近」而失去比對的意義(我們要模擬真實瀏覽器環境)。
...
兩端在數值上已經高度一致(Top-10 完全相同、ρ = 0.9998),代表把 MSD-MusiCNN 推論搬到前端 / Node 是可行的,輸出可以信任。剩下的微小浮點差異是不同 toolchain 的本質差異,在判定門檻內,屬於可接受範圍。
